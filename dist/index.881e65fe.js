...
function rescramble() {
    for(i = 0; i < this.indices.length; i++){
        ...
        scrambledStringTemp = this.string.substring(0, indexToMove) + this.string.substring(indexToMove + 1) + this.string.substring(indexToMove, indexToMove + 1);
...
function bubbleSortStep() {
    if (this.bubbleSortBookmark >= this.indices.length - 1) this.bubbleSortBookmark = 0;
    for(i = this.bubbleSortBookmark; i < this.indices.length - 1; i++){
        ...
        if (this.indices[i] > this.indices[i + 1]) {
            ...
            tempArrange = this.string.substring(0, i) + this.string.substring(i + 1, i + 2) + this.string.substring(i, i + 1) + this.string.substring(i + 2);
    ...
    this.bubbleSortBookmark = i;
    if (!this.changed) clearInterval(this.interval);
}

//# sourceMappingURL=index.881e65fe.js.map
